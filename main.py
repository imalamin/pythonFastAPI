...
@app.get("/blog")
async def index(limit: int, published: Optional[bool] = True, sort: Optional[str] = None):
    if published:
...
async def update_blog_by_id(id: int, request: schema.Blog, db: Session = Depends(get_db)):
